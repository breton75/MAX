...
QSqlError svidev::open_db(QString dbFileName)
{
  ...
  if(err.type() == QSqlError::NoError) {
    ...
    if(!q->next()) {
      ...
      err = SQLITE->execSQL("create table devices (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, "
                      "device_type INTEGER, device_name TEXT, vendor_id INTEGER, product_id INTEGER, "       
                      "manufacturer_id INTEGER, port INTEGER, date_time TEXT, description TEXT)");
...
{
  QSqlQuery* q = new QSqlQuery(SQLITE->db);
  QSqlError err = SQLITE->execSQL(QString("select id, device_name from devices order by date_time"), q);
...
{
  QSqlQuery* q = new QSqlQuery(SQLITE->db);
  QSqlError err = SQLITE->execSQL(QString("select * from devices where id = %1").arg(dbid), q);
