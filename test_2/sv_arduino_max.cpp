...
void svarduinomax::SvArduinoWidget::pullSensors()
{
  try {
    ...
    for(QString state: l) {
      ...
      if(state.startsWith("CURRENT:TEMP:")) {
        ...
        _seconds_passed = time(NULL) - _last_time;
        _current_angle_speed = qreal(_current_angle - _last_angle) / _seconds_passed;
        _current_turn_speed = (_current_angle_speed * 60) / 360.0;
  ...
  ui->textLog->setText(QString("Текущее состояние:\n  температура: %1\n  угол: %2\n  оборот: %3\nМгновенная скорость: %4 об./мин. (%5 град./сек.)")
                       .arg(_current_temperature).arg(_current_angle).arg(_current_turn).arg(_current_turn_speed).arg(_current_angle_speed));
